...
function doRectanglesOverlap(r1, r2) {
  ...
  const horiz = r1.left < r2Right && r2.left < r1Right;
  const vert = r1.top < r2Bottom && r2.top < r1Bottom;
...
function isCreditCardNumber(ccn) {
  ...
  const sorted = [];
  const arrCnn = str.split('').reverse();
  for (let i = 0; i < arrCnn.length; i += 1) {
    if (i % 2 === 0) {
      sorted.unshift(+arrCnn[i]);
    } else {
      sorted.unshift(+arrCnn[i] * 2);
    }
  }
  const result = sorted.map((el) => {
    let val = el;
    if (val > 9) val -= 9;
    return val;
  });
  return result.reduce((acc, el) => acc + el) % 10 === 0;
}
